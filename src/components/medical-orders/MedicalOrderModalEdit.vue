...
export default defineComponent({
  ...
  props: {
    ...
    data: { type: Array<MedicalOrder>, required: true },
    state: { type: Boolean, required: true }
  },
  emits: ['close', 'save', 'update', 'stateOf'],
  setup(props, { emit }) {
    ...
    const handleSubmit = async () => {
      ...
      if (isValid) {
        ...
        emit('update', JSON.stringify(order.value), props.id)
        emit('close')
        order.value = {
          name: '',
          lastName: '',
    ...
    onUpdated(() => {
      ...
      if (props.state) {
        order.value = {
          ...
          idNumber: props.data[0].idNumber,
          eps: props.data[0].eps,
          medicines: props.data[0].medicines,
          doctorSignature: props.data[0].doctorSignature,
          comentarios: props.data[0].comentarios
        }
        emit('stateOf')
      }
    })
